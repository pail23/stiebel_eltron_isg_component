...
ENERGY_SYSTEM_INFORMATION_REGISTERS = {
    EnergySystemInformationRegisters.SG_READY_OPERATING_STATE: ModbusRegister(
        ...
        max=4.0,
        data_type=6,
        key=EnergySystemInformationRegisters.SG_READY_OPERATING_STATE,
    ),
    EnergySystemInformationRegisters.CONTROLLER_IDENTIFICATION: ModbusRegister(
        ...
        max=None,
        data_type=6,
        key=EnergySystemInformationRegisters.CONTROLLER_IDENTIFICATION,
    ),
}
...
class StiebelEltronAPI:
    ...
    async def write_register_value(
        self, register: IsgRegisters, value: int | float
    ) -> None:
        ...
        if descriptor is not None:
            await self._client.write_register(
                ...
                value=self.convert_value_to_modbus(value, descriptor),
                slave=1,
            )
        else:
    ...
    def convert_value_to_modbus(
        self, value: int | float, register_description: ModbusRegister
    ) -> int:
        """Convert a modbus value to a python value."""
        if register_description.data_type == 2:
            register = self._client.convert_to_registers(
                [int(value * 10)], self._client.DATATYPE.INT16
            )
            return register[0]
        if register_description.data_type == 6:
            register = self._client.convert_to_registers(
                [int(value)], self._client.DATATYPE.UINT16
            )
            return register[0]
        if register_description.data_type == 7:
            register = self._client.convert_to_registers(
                [int(value * 100)], self._client.DATATYPE.INT16
            )
            return register[0]
        if register_description.data_type == 8:
            register = self._client.convert_to_registers(
                [int(value)], self._client.DATATYPE.UINT16
            )
            return register[0]
        ...
        for registerblock in self._register_blocks:
            ...
            if heat_pump_data is not None and not heat_pump_data.isError():
                ...
                for i in range(0, registerblock.count):
                    ...
                    if descriptor is not None:
                        result[descriptor.key] = self.convert_value_from_modbus(

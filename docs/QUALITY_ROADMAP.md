...
- [ ] Add `quality_scale.yaml` with evidence-backed `done` and `exempt`
  entries; do not claim a formal tier in `manifest.json`. Draft #634 follows
  #622.
...
- [ ] Replace the current README with structured end-user documentation:
  ...
  platform/use-case/example follow-up is in draft #634.
...
- [ ] Assign `EntityCategory.CONFIG` and `EntityCategory.DIAGNOSTIC` where the
  ...
  the wider audit are in draft #632.
...
- [ ] Audit every sensor for the most specific available device class, state
  class and unit. Pressure and volume-flow corrections are in draft #632;
...
- [ ] Move hardcoded entity icons into `icons.json`. The existing climate
  ...
  `icon-translations` rule. Draft #633 covers Number, binary-sensor and Sensor
  icons after the device-class work in #632.
...
- [ ] Define the single-device interpretation of dynamic/stale device rules and
  test config-entry removal and re-add behavior. Draft #634 contains a full
  lifecycle test and records both no-exception rules as done.
